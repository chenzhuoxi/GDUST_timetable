...
> 📱 **抓取完课表后**，推荐配合 [GDUST_timetable_lite](https://github.com/chenzhuoxi/GDUST_timetable_lite)（Flutter 课表查看 App）导入 JSON 随时查看，告别网页转圈圈！

## 它能做什么

- 输入学号 + 校园账号密码，自动登录 CAS 统一认证
- 支持 **扫码登录**（钉钉/微信扫码，无需输入验证码）
- 通过 WebVPN 代理请求教务系统课表接口
- 支持 **ddddocr 自动识别验证码**（全自动），也支持手动输入
- 按周抓取（第 1~20 周），增量合并到本地 JSON 文件
- **Web GUI**：浏览器操作，可视化界面，表格展示课表
- 导出 JSON 后可导入 [Flutter Lite 版](https://github.com/chenzhuoxi/GDUST_timetable_lite) 随时查看

## 典型用法

```
1. 本工具抓取课表 → 导出 timetable.json
2. 导入到 GDUST_timetable_lite App → 手机随时查看
```

| 步骤 | 工具 | 说明 |
|------|------|------|
| ① 抓取课表 | 本项目（Web GUI / 命令行） | 登录 → 抓取 → 导出 JSON |
| ② 查看课表 | [GDUST_timetable_lite](https://github.com/chenzhuoxi/GDUST_timetable_lite) | 导入 JSON → 手机随时看 |
...
### 📱 配套课表 App：[GDUST_timetable_lite](https://github.com/chenzhuoxi/GDUST_timetable_lite)

抓取完课表后，推荐导入 Flutter Lite 版 App 随时查看：

1. 用本工具抓取课表，下载 `timetable.json`
2. 手机安装 [GDUST_timetable_lite](https://github.com/chenzhuoxi/GDUST_timetable_lite) APK
3. App 内导入 JSON 文件 → 即可离线查看课表

**Lite 版特点**：纯展示，无需登录，导入即用，支持教学周自动切换
